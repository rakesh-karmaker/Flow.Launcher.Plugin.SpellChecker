...
class Main(FlowLauncher):
    ...
    def getDefinition(self, text: str):
        ...
        try:
            ...
            if (
                wordnet_definition == self.WORDNET_NOT_FOUND_TAG
                or wordnet_definition == self.NO_DEFINITION_TAG
            ):
                wordnet_error_note = (
                    " (WordNet is not installed thus the definition may be slow. Run: python -m nltk.downloader wordnet)"
                    if wordnet_definition == self.WORDNET_NOT_FOUND_TAG
                    else ""
                )
    ...
    def checkSpelling(self, text: str):
        ...
        if len(misspelled) == 0:
            ...
            if suggestions and len(suggestions) > 0:
                (
                    suggestions.remove(correct_spelling)
                    if correct_spelling in suggestions
                    else None
                )
            ...
            return [
                {
                    ...
                    "SubTitle": "You can also toggle definitions with '-d' or '-dw'.",
            ...
            if isText:
                match value.lower():
                    ...
                    case "-dw":
                        ...
                        self.MEANING_FINDER = (
                            "wikipedia"
                            if self.MEANING_FINDER == "default"
                            else "default"
                        )
                        continue
                    case _:
